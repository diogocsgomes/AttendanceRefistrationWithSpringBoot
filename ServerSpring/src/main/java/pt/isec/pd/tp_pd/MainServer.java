			...
			while (running) {
				...
				switch (receivedRequest.getType()) {
					...
					case ADMIN_DELETE_EVENT -> {
						...
						if (!hasAttendances) {
							int eventId = (int) receivedRequest.getRequestArguments();
							boolean isDeleted = databaseManager.deleteEvent(eventId);
...
public class MainServer extends UnicastRemoteObject implements Runnable, Remote, DatabaseBackupInterface, Serializable {
	...
	public MainServer(File databaseDirectory, String[]args) throws RemoteException {
		this.args = args;
	...
	public void run() {
		...
		try {
			// Creates the service
			MainServer mainServer = new MainServer(databaseDirectory, args);
			...
			databaseManager = DatabaseManager.getInstance(mainServer);
